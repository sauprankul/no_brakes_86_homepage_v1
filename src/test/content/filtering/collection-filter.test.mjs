...
test('an untouched index collection lists direct children only', () => {
  ...
  assert.deepEqual(ids(filterCollection({ direct, descendants, filters: defaults() })), ['article-direct', 'index-direct']);
});
...
test('an active query expands candidates to direct and indirect entries', () => {
  ...
  assert.deepEqual(ids(filterCollection({ direct, descendants, filters })), ['draft-nested', 'article-nested', 'index-direct']);
});

test('articles-only yes and no use public language while filtering both entry forms', () => {
  assert.deepEqual(ids(filterCollection({ direct, descendants, filters: { ...defaults(), articlesOnly: 'yes' } })), ['draft-nested', 'article-nested', 'article-direct']);
...
test('include/exclude tags, date bounds, and each order are applied together', () => {
  ...
  assert.deepEqual(ids(filterCollection({ direct, descendants, filters: { ...defaults(), includeTags: ['setup'], order: 'title' } })), ['draft-nested', 'index-direct', 'article-nested']);
});
