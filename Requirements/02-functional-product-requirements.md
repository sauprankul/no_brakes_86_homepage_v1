...
- The default production policy is **no analytics collection from the EEA, United Kingdom, or Switzerland**. The collector must discard those events before persistence. Do not turn this regional guard off without a release-specific, documented privacy assessment and the appropriate visitor flow for every affected market.
...
- Search popularity may retain a normalized term only when it is short and uses ordinary letters, numbers, spaces, `/`, `+`, or `-`; reject emails, URLs, phone-like strings and other unusual input. Treat even a permitted term as potentially personal data, suppress it from reports until it has at least five recorded searches, and never use it for profiling or advertising.
- Never collect or persist names, email addresses, request IP addresses, full referrers, cookies, ad IDs, device fingerprints, stable visitor IDs, MAC addresses, precise location, form input, raw user agent, or cross-site activity. The collector must not claim that all remaining telemetry is legally “anonymous”; it is deliberately minimized aggregate telemetry. The site must not attempt to read a MAC address: browsers do not expose it and it is not an appropriate web-analytics identifier.
- There is no public analytics opt-out URL, settings UI, or banner by default. An owner-only browser exclusion exists for this site’s operator; it is set locally using the `no-brakes:analytics-owner-excluded` browser-storage key and is not transmitted. If an applicable law or the release assessment requires an objection, opt-out, or consent mechanism, implement that mechanism before enabling analytics in that jurisdiction.
